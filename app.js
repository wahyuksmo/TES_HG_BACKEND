  ...
  app.post("/pembayaran", async (req, res) => {
    ...
    try {
      if (!Number.isInteger(amount_paid) || amount_paid <= 0) {
        return res.status(400).json({ error: "amount_paid harus bilangan bulat positif" });
      }
      ...
      if (omzet >= targetKomisi) {
        return res.status(400).json({ error: "Omzet sudah mencapai target komisi, tidak perlu pembayaran" });
      }
      ...
      if (amount_paid > remaining_balance) {
        return res.status(400).json({ error: "Pembayaran melebihi sisa yang dibutuhkan untuk mencapai komisi" });
      }
